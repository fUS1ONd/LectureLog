...
def evidence_matches_entry(evidence: str, entry: SlideCatalogEntry) -> bool:
    ...
    for claim in _claims(entry):
        ...
        if (
            ...
            and (
                len(claim_tokens) >= 2
                or _is_distinctive_singleton(next(iter(claim_tokens)), claim, entry)
            )
        ):
            return True
        ...
        if len(overlap) >= 2:
            return True
        if len(overlap) == 1 and _is_distinctive_singleton(next(iter(overlap)), claim, entry):
            return True
    return False


def evidence_specificity(evidence: str, entry: SlideCatalogEntry) -> tuple[int, int, float]:
    ...
    for claim in _claims(entry):
        ...
        overlap = len(shared)
        distinctive = overlap == 1 and _is_distinctive_singleton(next(iter(shared)), claim, entry)
        exact_phrase = len(claim_tokens) >= 2 and _normalize(claim) in _normalize(evidence)
...
def _claims(entry: SlideCatalogEntry) -> tuple[str, ...]:
    visible_claims = tuple(
        part.strip() for part in re.split(r"[\n•;]+", entry.visible_text) if part.strip()
    )
    return tuple(
        value
        for value in (
...
def _is_distinctive_singleton(token: str, claim: str, entry: SlideCatalogEntry) -> bool:
