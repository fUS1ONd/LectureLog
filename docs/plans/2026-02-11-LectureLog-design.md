...
1. **Нормализация громкости** — фильтр `loudnorm` (EBU R128), двухпроходный режим (анализ → применение)
2. **Шумоподавление** — нейросетевой фильтр `arnndn` (RNNoise), автоматическая адаптация без настраиваемых параметров
3. **Вырезание тишины** — `silencedetect` + `atrim`/`concat`, порог и минимальная длительность из пресета, сохранение маппинга таймкодов (оригинал → обрезанное)

Каждый этап можно включить/отключить независимо.

#### Пресеты (MVP)

| Пресет | Описание | loudnorm | arnndn (RNNoise) | silence_thresh | silence_duration |
|--------|----------|----------|------------------|----------------|------------------|
| `quiet_room` | Тихое помещение, диктофон рядом | I=-24, LRA=7, TP=-2 | авто | -40dB | 5s |
| `lecture_hall` | Аудитория среднего размера | I=-24, LRA=11, TP=-2 | авто | -35dB | 5s |
| `noisy` | Шумное помещение, запись издалека | I=-24, LRA=14, TP=-2 | авто | -30dB | 4s |
| `custom` | Ручная настройка всех параметров | пользовательские | авто | пользовательские | пользовательские |

#### Архитектура библиотеки

```
audiopreproc/
├── cmd/audiopreproc/    # CLI-обёртка
├── models/              # Модель RNNoise (rnnoise-std.rnnn)
├── preset.go            # Определение пресетов
├── pipeline.go          # Оркестрация этапов
├── normalize.go         # Нормализация громкости
├── denoise.go           # Шумоподавление (RNNoise/arnndn)
├── silence.go           # Детекция и вырезание тишины + TimeMap
├── audiopreproc.go      # Базовые типы (Segment, TimeMap, Result)
└── ffmpeg.go            # Обёртка над вызовами ffmpeg
```

#### Маппинг таймкодов

При вырезании тишины создаётся `TimeMap` — массив сегментов `{original_start, original_end, processed_start, processed_end}`. Позволяет конвертировать таймкоды в обе стороны: из обработанного аудио → в оригинальное и наоборот. Используется при нарезке аудио по разделам на этапе 6.

#### CLI-интерфейс

```bash
# С пресетом
audiopreproc --preset lecture_hall input.mp3 output.mp3

# Кастомные параметры
audiopreproc --preset custom --silence-thresh -35 --silence-dur 3 input.mp3 output.mp3

# Отключение отдельных этапов
audiopreproc --preset lecture_hall --no-silence input.mp3 output.mp3

# Только шумоподавление
audiopreproc --no-normalize --no-silence --preset noisy input.mp3 output.mp3
```
