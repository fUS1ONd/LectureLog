...
2. **Шумоподавление чанков** — Python-скрипт `scripts/denoise.py` отправляет каждый чанк в HuggingFace Space `ResembleAI/resemble-enhance` через `gradio_client`
3. **Склейка результата** — `ffmpeg` собирает обработанные чанки обратно в один выходной файл
...
├── ffmpeg.go            # CheckFFmpeg, splitChunks, concatChunks
├── scripts/
