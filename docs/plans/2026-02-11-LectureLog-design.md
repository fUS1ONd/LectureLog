...
- **Нормализация громкости** — фильтр `loudnorm`, выравнивает скачки громкости
- **Шумоподавление** — фильтр `afftdn`, убирает фоновый шум аудитории
- **Вырезание тишины** — `silencedetect` + порог (по дефолту 5 сек), сохранение маппинга таймкодов (оригинал → обрезанное)
